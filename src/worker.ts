    ...
    if (job.data.songId == undefined) {
      if (job.data.searchString == undefined) {
        ...
        if (sanityCheck?.isNone) {
            throw new Error("No Searchs Results match for " + +job.data.searchString.replace(/[\[][a-zA-Z]*[\]]/ig, ""))
        }
 ...
 */
const lyricsFetcherQueue = new Worker('lyricsFetcher', async job => {
    const chain = job.data.chain ?? false;
